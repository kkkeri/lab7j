...
public class Server {
    ...
    private static Deque<String> lastEightCommands = new LinkedList<>();
    ...
    private void otherCommandHandler(String command, Scanner scanner) throws IOException {
        try {
            if (!command.isEmpty()) {
                synchronized (lastEightCommands) {
                    if (lastEightCommands.size() >= 8) {
                        lastEightCommands.removeLast();
                    }
                    lastEightCommands.addFirst(command);
                }
                Flat flat = new Flat();
                String key = null;
                boolean isClientCommand = false;
                if (command.equals("add") || command.equals("addIfMax") || command.equals("addIfMin")) {
                    flat = FlatGenerator.createFlat();
                    ...
                    if (command.split(" ").length == 1) {
                        HistoryCommand.execute(lastEightCommands);
                        isClientCommand = true;
                    } else {
                ...
                if (!isClientCommand) {
                    Request request = new Request(command, flat, key, Client.getInstance().getName(), Client.getInstance().getPasswd());
                    sendRequest(request);
                }
            }
        } catch (IOException e) {
