            ...
            switch (parts[0]) {
                case "add":
                    try {
                        ...
                        for (int i = 0; i < flatData.length; i++) {
                            if (i == 6) {
                                flatData[i] = new Date(0).toString();
                            } else {
                                flatData[i] = br.readLine();
                                try {
                                    switch (i) {
                                        case 0:
                                            Validator.validateName(flatData[i], "name");
                                            break;
                                        case 1:
                                            Validator.validateName(flatData[i], "x");
                                            Validator.coordinateX(flatData[i]);
                                            break;
                                        case 2:
                                            Validator.validateName(flatData[i], "y");
                                            Validator.coordinatesY(flatData[i]);
                                        case 3:
                                            Validator.validateName(flatData[i], "engine power");
                                            Validator.validateFurnish(flatData[i]);
                                            break;
                                        case 4:
                                            Validator.validateName(flatData[i], "vehicle type");
                                            Validator.validateArea(flatData[i]);
                                            break;
                                        case 5:
                                            Validator.validateName(flatData[i], "fuel type");
                                            Validator.validateTransport(flatData[i]);
                                            break;
                                    }
                        ...
                        Server.sendRequest(new Request("add", new Flat(flatData), null, Client.getInstance().getName(), Client.getInstance().getPasswd()));
                        flatData = new String[10];
                ...
                case "updateId":
                    try {
                        ...
                        Server.sendRequest(new Request("updateId " + id, new Flat(flatData), strId, Client.getInstance().getName(), Client.getInstance().getPasswd()));
                        flatData = new String[10];
                        isSpecialCommandExecuted = true;
                ...
                case "addIfMax":
                    try {
                        ...
                        Server.sendRequest(new Request("addIfMax", new Flat(flatData), null, Client.getInstance().getName(), Client.getInstance().getPasswd()));
                        flatData = new String[10];
                        isSpecialCommandExecuted = true;
                    } catch (NumberFormatException e) {
                        System.out.println("ID must be a valid number: " + e.getMessage());
